 ...
 */
App.controller('serviceRequestsController', ['$scope', '$http','Api','$state','$timeout','$cookieStore','$mdDialog','$mdMedia','$loading','$rootScope',
    function($scope,$http,Api,$state,$timeout,$cookieStore,$mdDialog,$mdMedia,$loading,$rootScope) {

        $rootScope.changecolor('serviceRequests');

        $loading.start('serviceRequests');
        $scope.customFullscreen = $mdMedia('xs') || $mdMedia('sm');

        angular.element(document).ready(function(){
            angular.element('#serviceRequestsPage').css('min-height', angular.element(window).height() -55 - 60+'px');
        ...
        $scope.getbookings = function(){
            $http({
                ...
                if ($.fn.DataTable.isDataTable("#serviceRequests")) {
                    $('#serviceRequests').DataTable().clear().destroy();
                }
                $scope.bookingsList = dataArray;
                datatable();
                $loading.finish('serviceRequests');
            }).error(function(data){
        ...
        var datatable = function(){
            ...
            $timeout(function(){
                ...
                dtInstance = $('#serviceRequests').dataTable({
        ...
        $scope.assignDriver = function(ev,data) {
            ...
            $mdDialog.show({
                ...
                templateUrl: 'app/views/popups/driverAssign.html',
                parent: angular.element(document.body),
                targetEvent: ev,
                clickOutsideToClose:true,
                fullscreen: useFullScreen,
