 ...
 */
App.controller('ongoingController', ['$scope', '$http','Api','$state','$timeout','$cookieStore','$mdDialog','$mdMedia','$loading','$rootScope',
    function($scope,$http,Api,$state,$timeout,$cookieStore,$mdDialog,$mdMedia,$loading,$rootScope) {

        $rootScope.changecolor('jobs');

        $loading.start('ongoing');
        $scope.customFullscreen = $mdMedia('xs') || $mdMedia('sm');

        angular.element(document).ready(function(){
            angular.element('#ongoingPage').css('min-height', angular.element(window).height() -55 - 60+'px');
        ...
        $scope.getbookings = function(){
            $http({
                ...
                if ($.fn.DataTable.isDataTable("#ongoing")) {
                    $('#ongoing').DataTable().clear().destroy();
                }
                $scope.bookingsList = dataArray;
                datatable();
                $loading.finish('ongoing');
            }).error(function(data){
